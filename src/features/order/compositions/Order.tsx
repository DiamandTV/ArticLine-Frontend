                    ...
                    <Accordion.Body className="w-full box-border p-0">
                        <Card className="w-full flex flex-col justify-start items-start px-2 py-2 rounded-none">
                            ...
                            <hr className="w-full my-1 h-0.5"/>
                                ...
                                <Order.DeliveryTimeDetails className="text-sm gap-2 text-surface-a10 font-medium"/>
                            </div>
...
Order.Status = function Status() {
  const { order } = useOrderContext();
  return <span className="font-medium p-0">{order.status}</span>;
};
  ...
  return (
    <div
      ...
      className={tailwindMerge(
        `px-2 py-1 rounded-lg text-white text-xs font-semibold ${bgColor}`,
        attr.className
      )}
    >
      <Order.Status />
...
Order.Temperature = function Temperature(attr: React.HTMLAttributes<HTMLElement>) {
  const { order } = useOrderContext();
  return (
    <div {...attr} className={tailwindMerge("text-xs font-medium text-gray-600", attr.className)}>
      {order.temperature_start_range}° - {order.temperature_end_range}°
    </div>
  );
};

Order.Icons = {
  Delivery: function DeliveryIcon(attr: React.HTMLAttributes<HTMLElement>) {
    return <div {...attr} className={tailwindMerge("text-xl", attr.className)}><IoCalendarOutline /></div>;
  },
  Delay: function DelayIcon(attr: React.HTMLAttributes<HTMLElement>) {
    return <div {...attr} className={tailwindMerge("text-xl", attr.className)}><LuClockAlert /></div>;
  },
  Address: function AddressIcon(attr: React.HTMLAttributes<HTMLElement>) {
    return <div {...attr} className={tailwindMerge("text-xl", attr.className)}><GoLocation /></div>;
  },
};
...
Order.DeliveryTimeDetails = function DeliveryTimeDetails(attr: React.HTMLAttributes<HTMLElement>) {
  const { order } = useOrderContext();
  if (!order.delivery_time) return null;
  const deliveryTime = new Date(order.delivery_time).toLocaleDateString();
  return (
    <div {...attr} className={tailwindMerge("flex flex-row items-center gap-1 text-xs text-gray-500", attr.className)}>
      <Order.Icons.Delivery />
      <span>{deliveryTime}</span>
    </div>
  );
};
...
Order.DelayTimeDetails = function DelayTimeDetails(attr: React.HTMLAttributes<HTMLElement>) {
  ...
  return (
    order.delay_time && (
      <div {...attr} className={tailwindMerge("flex flex-row items-center gap-1 text-xs text-orange-500", attr.className)}>
        <Order.Icons.Delay />
        <span>+{order.delay_time}</span>
      </div>
    )
  );
};
...
Order.Urgent = function Urgent(attr: React.HTMLAttributes<HTMLElement>) {
  const { order } = useOrderContext();
  return (
    order.request_earliest_delivery && (
      <div {...attr} className={tailwindMerge("flex items-center text-orange-500 text-xs font-semibold", attr.className)}>
        <IoAlertCircleOutline className="mr-1" /> Urgente
      </div>
    )
  );
...
Order.CreatedAt = function CreatedAt(attr:React.HTMLAttributes<HTMLElement>){
    const {order} = useOrderContext()
    return(
        <div {...attr} className={tailwindMerge("text-xs text-gray-500")}>
            Order created at {" "}
