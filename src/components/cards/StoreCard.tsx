...
export function StoreCard({store:{id,title,images,average_rating,ratings,views},style,className}:StoreCardProps){
    ...
    return (
        <Link to={`/store/details/${id}/`}>
            <div className={`w-full h-full rounded-xl hover:cursor-pointer ${className}}`} style={style} ref={divRef}>
                {/* IMAGES OF THE STORE CARD. MAX 5 IMAGES ARE SUPPORTED*/}
                <div className="w-full h-60 bg-transparent bg-no-repeat bg-center bg-cover rounded-t-lg" style={{backgroundImage:`url(${images[0]?.image})`}}>
